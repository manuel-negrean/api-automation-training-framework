...
public class PetSteps {
    ...
    public void thePetHasTagWithId(String tagName, int tagId) {
        petDto.setTags(java.util.Collections.singletonList(new PetDto.TagDto(tagId, tagName)));
    ...
    public void theResponseCodeShouldBe(int expectedResponseCode) {
        Assert.assertEquals(response.getStatusCode(), expectedResponseCode, "Was expected " + expectedResponseCode + " but was found" + response.statusCode());
    }

    @And("the response should contain the pet with name {string}")
    public void theResponseShouldContainThePetWithName(String expectedName) {
        PetDto responsePet = response.as(PetDto.class);
        Assert.assertEquals(responsePet.getName(), expectedName, "The expected name " + expectedName + " was not found" + responsePet.getName());
    }
}
