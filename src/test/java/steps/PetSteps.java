...
public class PetSteps {
    ...
    public void iHaveAPetWithNameAndStatus(String name, String status) {
        long id = System.currentTimeMillis(); // Generate a unique ID based on the current time
        petDto = new PetDto();
        petDto.setId(id);
        petDto.setName(name);
    ...
    public void theResponseCodeShouldBe(int expectedResponseCode) {
        Assert.assertEquals(response.getStatusCode(), expectedResponseCode, "Was expected " + expectedResponseCode +
                " but was found " + response.statusCode());
    }
    ...
    public void theResponseShouldContainThePetWithName(String expectedName) {
        ...
        Assert.assertEquals(responsePet.getName(), expectedName, "The expected name " + expectedName + "but was found"
        + responsePet);
